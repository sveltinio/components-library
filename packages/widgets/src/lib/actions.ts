...
export interface ClickOutsideActionOptions {
	enabled: boolean;
	cb?: (node: HTMLElement) => void;
}

export const clickOutsideAction: Action<HTMLElement, ClickOutsideActionOptions> = (
	node,
	options
) => {
	function eventHandler(event: MouseEvent) {
		...
		if (node && !node.contains(target) && !event.defaultPrevented) {
			if (options?.cb) options.cb(node);
		}
	}

	if (options?.enabled) window.addEventListener('click', eventHandler);
