<script lang="ts">
	...
	export let title = '';
	export let visibility: BreakpointsQueries = {};

	let titleTxt = title == '' ? alt : title;
	let innerWidth = 0;

	const defaultVisibilityQueries: BreakpointsQueries = {
		xs: false,
		sm: false,
		md: true,
		lg: true,
	...
	const visibilityQueries = merge(defaultVisibilityQueries, visibility);
	...
	const isVisibleOn = (breakpoint: BreakpointMatch) =>
		getPropertyValue(visibilityQueries, breakpoint);
